...
```
weiji-app (:17900 H5)         →   weiji-server (:17801)   →   weiji-ai (:17802)
weiji-admin-web (:17901)  ─┘      cool-admin-midway           FastAPI AI 服务
...
| 服务 | 端口 | 技术栈 | 职责 |
|------|------|--------|------|
| [weiji-server](file:///workspace/weiji-server) | 17801 | cool-admin-midway（Midway.js + TypeORM + MySQL + Redis + `@cool-midway/core`） | 业务后端，base 模块 + 9 个业务模块（account/record/family/achievement/checkin/challenge/gamification/ai/analytics） |
| [weiji-admin-web](file:///workspace/weiji-admin-web) | 17901 | cool-admin-vue（Vue3 + Vite + Element Plus + cl-crud/cl-form） | PC 管理后台，9 业务页面 + cl-crud 管理页 |
| [weiji-app](file:///workspace/weiji-app) | 17900 | cool-uni（uni-app + Vue3 组合式 + Pinia） | 移动端 H5 / 微信小程序，8 页面 + Gameplay |
| [weiji-ai](file:///workspace/weiji-ai) | 17802 | FastAPI + httpx + AsyncOpenAI | AI 服务层，6 家厂商集成 + 三级降级 |
...
启动后 cool-admin 自动建表并加载各模块 `db.json` 种子数据；AI 代理转发 `/app/ai/*` → `http://localhost:17802`。
...
打开浏览器访问 `http://localhost:17901`，后台用 `admin/123456` 登录（cool-admin 内置）；C 端业务页用演示账号 `demo/123456`。
...
打开浏览器访问 `http://localhost:17900`，使用演示账号登录。
...
```
workspace/
├── weiji-server/             # cool-admin-midway 业务后端 (:17801)
│   ├── src/
...
│   └── package.json
├── weiji-admin-web/          # cool-admin-vue PC 后台 (:17901)
│   ├── src/
...
│   ├── vite.config.ts           # proxy /admin、/app → :17801
│   └── package.json
├── weiji-app/                # cool-uni 移动端 (:17900 H5)
...
│   └── package.json
├── weiji-ai/                 # FastAPI AI 服务 (:17802，独立 AI 层，不变)
...
| AI 代理 | `/app/ai/*` | 代理转发到 weiji-ai:17802，30s 超时 + 降级 |
