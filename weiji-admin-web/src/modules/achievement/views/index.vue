<template>
  <div class="page-content">
    ...
    <div v-loading="badgesLoading" class="badge-grid">
      ...
      >
        <div class="badge-icon" :style="{ background: b.unlocked ? badgeColor(b) : 'var(--el-fill-color-light)' }">
...
async function loadLevel() {
  ...
  try {
    const data: any = await appApi.getLevel();
    level.value = data || {};
  } catch (err: any) {
...
async function loadBadges() {
  ...
  try {
    const data: any = await appApi.getAchievements();
...
async function loadChallenges() {
  ...
  try {
    const data: any = await appApi.getChallenges();
...
function badgeColor(b: any) {
  return b.color || 'var(--el-color-primary-light-9)';
}
