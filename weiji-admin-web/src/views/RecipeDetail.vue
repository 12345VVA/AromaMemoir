      ...
      <div v-if="recipe" class="wj-card detail-card">
        ...
        <div class="detail-info">
          ...
          <div class="detail-author">
            ...
            <span>作者ID：{{ recipe.uploaderId || '未知' }}</span>
...
const isAuthor = computed(
  () => !!recipe.value && !!currentUserId.value && recipe.value.uploaderId === currentUserId.value
);
...
async function loadDetail() {
  ...
  loading.value = true;
  try {
    const data: any = await api.getRecipeDetail(id);
    recipe.value = data;
  } catch (err: any) {
