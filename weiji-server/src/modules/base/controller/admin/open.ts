...
export class BaseOpenController extends BaseController {
  ...
  public async getEps() {
    const admin = this.eps.admin as Record<string, any[]>;
    // 按 prefix 深度排序：短路径（父）在前，长路径（子）在后。
    // 修复 cool-admin-vue vite-plugin createService 处理嵌套 admin 路径时
    // "Cannot set properties of undefined" 崩溃（子先于父处理导致父节点缺 permission）
    const sorted: Record<string, any[]> = {};
    for (const moduleName of Object.keys(admin)) {
      sorted[moduleName] = [...admin[moduleName]].sort(
        (a, b) =>
          (a.prefix || '').split('/').filter(Boolean).length -
          (b.prefix || '').split('/').filter(Boolean).length
      );
    }
