...
export function getUserMembership(userId: string): FamilyMember | undefined {
  return findByField(family_members, 'userId', userId);
}

// 查找用户所属家庭组（未删除）
// 流程：先查 family_members 拿到 familyId，再查 families 并排除已软删除的家庭
// 未加入任何家庭组时返回 null
export function findUserFamily(userId: string): Family | null {
  const membership = getUserMembership(userId);
  if (!membership) return null;
  const family = findByField(families, 'id', membership.familyId);
...
export function requireRole(userId: string, roles: UserRole[]): boolean {
  const membership = getUserMembership(userId);
