...
describe('AchievementService.checkAndUnlockRecordAchievements', () => {
  it('demo 用户 first_record 已解锁、其余 record/variety 未达标 → 返回空数组', async () => {
    ...
    const unlocked = await AchievementService.checkAndUnlockRecordAchievements(DEMO_USER_ID);
...
describe('AchievementService.checkAndUnlockStreakAchievements', () => {
  it('streak=7 时 streak_7 已解锁、streak_30 未达标 → 返回空数组', async () => {
    // 种子数据：demo 已解锁 ach-0002(streak_7)
    const unlocked = await AchievementService.checkAndUnlockStreakAchievements(DEMO_USER_ID, 7);
    assert.ok(Array.isArray(unlocked));
    assert.strictEqual(unlocked.length, 0, '应返回空数组');
  });

  it('streak=30 时 streak_30 达标 → 返回 1 条 ach-0003', async () => {
    const unlocked = await AchievementService.checkAndUnlockStreakAchievements(DEMO_USER_ID, 30);
    assert.ok(Array.isArray(unlocked));
...
describe('AchievementService.checkAndUnlockFamilyAchievements', () => {
  it('demo 是 family-0001 owner → family_create 解锁 ach-0006', async () => {
    const unlocked = await AchievementService.checkAndUnlockFamilyAchievements(DEMO_USER_ID);
    assert.ok(Array.isArray(unlocked));
    assert.strictEqual(unlocked.length, 1, '应解锁 1 条');
  ...
  it('再次调用 family 类检查 → 幂等返回空数组', async () => {
    // 上一次已解锁 family_create，重复调用不应再产生记录
    const beforeArr = await user_achievements.findAll(
    ...
    const afterArr = await user_achievements.findAll(
      (ua) => ua.userId === DEMO_USER_ID && ua.achievementId === 'ach-0006',
    );
    const afterCount = afterArr.length;
...
describe('AchievementService.checkAndUnlockGameplayAchievements', () => {
  it('blindguess_chef 直接解锁 → 返回 1 条 ach-0007', async () => {
    const unlocked = await AchievementService.checkAndUnlockGameplayAchievements(DEMO_USER_ID);
    assert.ok(Array.isArray(unlocked));
    assert.strictEqual(unlocked.length, 1, '应解锁 1 条');
